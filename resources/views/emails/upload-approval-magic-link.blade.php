...
<title>Persetujuan Upload Dokumen</title>
<style>
  ...
  .doc-card .value { font-size:14px; color:#0d2b4e; font-weight:600; }
  .badge-sr { display:inline-block; background:#fee2e2; color:#7f1d1d; padding:3px 10px; border-radius:12px; font-size:12px; font-weight:700; }
  ...
  .note { background:#fffaf0; border-left:4px solid #c8972a; padding:12px 16px; border-radius:0 6px 6px 0; font-size:12px; color:#744210; margin-top:16px; }
  .divider { border:none; border-top:1px solid #e5e7eb; margin:24px 0; }
  .step-info { font-size:12px; color:#6b7280; text-align:center; margin-bottom:8px; }
  .previous-approval { background:#f0fdf4; border:1px solid #bbf7d0; border-radius:6px; padding:10px 14px; font-size:12px; color:#166534; margin-bottom:16px; }
...
<div class="container">
    ...
    <div class="header">
        ...
        <p>Permintaan Persetujuan Dokumen</p>
    </div>

    <div class="body">
        <p>Yth. <strong>{{ $step->approver_name }}</strong>,</p>
        <p>
            Terdapat dokumen berkategori <span class="badge-sr">Sangat Rahasia</span>
            yang memerlukan persetujuan Anda sebelum dapat diarsipkan di sistem.
        </p>

        {{-- Info dokumen --}}
        <div class="doc-card">
            <div class="label">Nomor Surat</div>
            <div class="value">{{ $approval->document->document_number }}</div>
            <div class="label mt-2">Perihal / Judul</div>
            <div class="value">{{ $approval->document->title }}</div>
            <div class="label mt-2">Tanggal Dokumen</div>
            <div class="value">{{ $approval->document->document_date->translatedFormat('d F Y') }}</div>
            <div class="label mt-2">Divisi</div>
            <div class="value">{{ $approval->document->division->name ?? '-' }}</div>
            <div class="label mt-2">Diajukan Oleh</div>
            <div class="value">{{ $approval->requester->name }} pada {{ $approval->requested_at->translatedFormat('d F Y, H:i') }}</div>
        </div>

        {{-- Tampilkan bukti step sebelumnya jika ini step 2 --}}
        @if($step->step_order === 2)
            @php
                $prevStep = $approval->steps->firstWhere('step_order', 1);
            @endphp
            @if($prevStep && $prevStep->status === 'approved')
            <div class="previous-approval">
                ✅ <strong>Bagian Umum sudah menyetujui</strong> pada
        ...
        {{-- Tombol aksi --}}
        <a href="{{ url('/approve/upload/' . $step->token . '/approve') }}" class="btn-approve">
            ✅ &nbsp; Setujui Dokumen Ini
        </a>
        <a href="{{ url('/approve/upload/' . $step->token . '/reject') }}" class="btn-reject">
            ❌ &nbsp; Tolak Dokumen Ini
        </a>
        ...
        <div class="note">
            ...
            Jika Anda tidak merasa mengajukan persetujuan ini, abaikan email ini.
        </div>
        ...
        <p class="token-note">
            ...
            <code style="font-size:10px; word-break:break-all;">
                {{ url('/approve/upload/' . $step->token . '/approve') }}
            </code>
        </p>
    </div>
